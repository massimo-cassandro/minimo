 ...
 * @param {Function} datatypes.sortValue - the value used for sorting. If not defined, the raw value is used
 * @param {Function} datatypes.searchValue - the value used for searching. If not defined, the raw value is used
...
export function defaultDataTypes(params) {
  ...
  const inlineEndAlignClassName  = params.inlineEndAlignClassName?? styles.endAlign
    ...
    ,date_datetime_datatype = isDatetime => {
      ...
      return {
        ...
        sortValue: (d/* , global_params */) => parseDateString(d)?.toISOString()?? '',
        searchValue: (d, global_params) => {
  ...
  const datatypes =  {
    ...
    num: {
      ...
      sortValue        : val => parseNum(val),
      searchValue      : val => parseNum(val),
