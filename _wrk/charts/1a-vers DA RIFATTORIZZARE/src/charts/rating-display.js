...
export async function ratingDisplay({
  ...
  /** se true aggiunge width e height */
  addSvgSize = true,
  ...
  */
  ...
  /**
    ...
    displayLabel: [
      {
        label: <text> || null,
        font: obj || null, // oggetto analogo a this.defaults.font
        fill: <color>,
    ...
    miniDisplay = [
      {
        ...
        mdArc: {                   // oggetto simile a displayArc, è relativo all'arco che avvolge il mini display
          fill: null,
          strokeWidth: null,
          strokeColor: null,
          className: null,
  ...
  */
  ...
  try {
    ...
    scaleColors??= this.defaults.colors;
    ...
    const rad30 = this.utils.toRadians(30),
      ...
      getSanitizedColorIndex = value => Math.max(
        ...
        Math.min(
          scale - 1,
          Math.floor(value) - displayScaleStartValue
        )
      ),
    ...
    const sizes = {
      ...
      mainDisplay: {
        ...
        // r: 110,
        get r() {
          return 110 -  ((displayArc?.strokeWidth??0) / 2);
        },

        get w() {
          return this.r * 2 +
            // spessore traccia arco esterno, se presente
            (displayArc?.strokeWidth??0);
        },
        /*
        ...
        */
        get cx() {
          return this.r + ((displayArc?.strokeWidth??0) / 2);
        },
        get cy() {
          return this.r + ((displayArc?.strokeWidth??0) / 2);
        }
    ...
    */
    const parsedDisplayValue = displayValueForceCenter? Math.floor(displayValue) + .499 : displayValue;
    ...
    if(animation) {
      ...
      rotazioneAsticella = Math.max(0, ((parsedDisplayValue  - displayScaleStartValue) * 240) / (scale - .001));
    }
    ...
    mainDisplayGroup.path(
      ...
      .attr({
        ...
        'stroke-linecap': displayArc?.strokeWidth? 'butt' : null,
      });
    ...
    rodGroup.polygon('59.1813 139.0943 59.1813 139.0943 98.8572 108.491 105.5448 120.0899 59.1813 139.0943')
      ...
      .attr({
        'data-debug-info': debug? `Value: ${parsedDisplayValue}, color index: ${getSanitizedColorIndex(parsedDisplayValue)},  {Angolo: ${rotazioneAsticella}, Start value: ${displayScaleStartValue}` : null,
      });
    ...
    for await (const md of miniDisplay) {
      ...
      if(md.type === 'value') {
        ...
        const [value_bbox, valueEl] = await this.createTextEl ({
      ...
      // =>> modalità "gauge"
        ...
        const segmentStrokeWidth = 6
          ...
          ,segmentAngle = this.utils.toRadians(240/scale - 30)
        ...
        segmentGroup.transform({
          ...
          rotate: (240/scale) * getSanitizedColorIndex(md.value) - displayScaleStartValue
        });
      }
    ...
    if(addSvgSize) {
      svgCanvas.size(sizes.mainSvg.w, sizes.mainSvg.h);
    }
