...
import { defaults_params, columns_default } from './defaults';
import { defaultDataTypes } from './default-columns-data-types';
...
export function parseParams({custom_params = {}, custom_data_types = {}}) {
  ...
  try {
    ...
    if(!params.container) {
      throw '`container` parameter not present';
    }
    if(typeof params.container === 'string') {
      throw '`container` parameter must be a dom element';
    }
