...
import static_demo_data  from '../demo-data/json-static-table-demo-data.js';
...
const container = document.getElementById('static-table-demo');
...
new jsonTable(
  // params
  {
    ...
    footerData: [
      {
        'id': 'this will not be rendered, sice the the `tfoot` option in `columns` is set to false',
        'isActive': null,
        'number1': static_demo_data.reduce((prev, curr) => prev + curr.number1, 0),
        'number2': static_demo_data.reduce((prev, curr) => prev + curr.number2, 0),
        'sum:': static_demo_data.reduce((prev, curr) => prev + curr.number1 + curr.number2, 0),
        'date': null,
        'datetime': null,
        'text': null,
        'perc': null,
        'percDecimal': null,
        'euro': static_demo_data.reduce((prev, curr) => prev + curr.euro, 0),
      }
