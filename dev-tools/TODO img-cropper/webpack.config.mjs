// webpack.config.mjs img-cropper
...
const isDevelopment = process.env.NODE_ENV === 'development'
  ...
  ,favicons_path = 'app/favicons/output'
...
const shared_chunk_paths =
  new RegExp([].map(p => `${sep}${p.replace(/\//g, sep)}${sep}`).join('|')
  );
...
const CopyWebpackPluginPatterns = [
  {
    from: 'app/favicons/output/icon-*.png',
    to: '[name][ext]'
  },
  ...
  */
  ...
  plugins: [
    ...
    new HtmlWebpackPlugin({
      filename: 'index.html',
      template: path.resolve(__dirname, './app/tpl/webpack-template.ejs'),
      inject: 'body',
      title: 'img-crop',
    ...
    new HtmlWebpackPlugin({
      ...
      template: path.resolve(__dirname, './app/favicons/output/manifest.webmanifest.ejs'),
      inject: false,
